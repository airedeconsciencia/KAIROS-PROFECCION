...
class SwissEph {
  ...
  async initSwissEph() {
    ...
    console.log("🛠️ SwissEph: Preparing WASM module configuration...");
    ...
    if (typeof process !== 'undefined' && process.versions && process.versions.node) {
      try {
        ...
        moduleConfig.locateFile = (path, prefix) => {
          if (path.endsWith('.data') || path.endsWith('.wasm')) {
            const resolvedPath = join(__dirname, '../wasm', path);
            console.log(`🔍 SwissEph (Node): Resolving ${path} to ${resolvedPath}`);
            return resolvedPath;
          }
          return prefix + path;
        };
      ...
      moduleConfig.locateFile = (path, prefix) => {
        if (path.endsWith('.data') || path.endsWith('.wasm')) {
          const resolvedPath = new URL('../wasm/' + path, import.meta.url).href;
          console.log(`Bitácora: Resolving ${path} to ${resolvedPath}`);
          return resolvedPath;
        }
        return prefix + path;
      };
    ...
    console.log("🛠️ SwissEph: Calling WasmSwissEph(moduleConfig)...");
    try {
        this.SweModule = await WasmSwissEph(moduleConfig);
        console.log("🛠️ SwissEph: WasmSwissEph resolved SUCCESSFULLY.");
    } catch (err) {
        console.error("❌ SwissEph: FATAL ERROR during WasmSwissEph execution:", err);
        throw err;
    }
