...
(function () {
  ...
  const PLANET_DEFS = { sol: '#B45309', luna: '#7C3AED', mercurio: '#0369A1', venus: '#BE185D', marte: '#B91C1C', jupiter: '#D97706', saturno: '#4B5563', urano: '#0E7490', neptuno: '#106191', pluton: '#6B21A8' };
  ...
  function buildPlanets(data) {
    ...
    const pList = Object.keys(planets).map(k => {
      ...
      const abs = (signToDeg(p.sign) + parseFloat(p.degree) + (p.minute || 0) / 60) % 360;
      return { k, p, abs, vis: getVisualDeg(abs, ascAbs) };
    ...
    clusters.forEach(c => {
      ...
      c.forEach((item, idx) => {
        ...
        const glifoKey = { 'SOL': 'Sol', 'LUNA': 'Luna', 'MERCURIO': 'Mercurio', 'VENUS': 'Venus', 'MARTE': 'Marte', 'JUPITER': 'Júpiter', 'SATURNO': 'Saturno', 'URANO': 'Urano', 'NEPTUNO': 'Neptuno', 'PLUTON': 'Plutón' }[item.k.toUpperCase()] || item.k;
        ...
        out += `<text x="${pText.x}" y="${pText.y}" text-anchor="middle" font-size="7" fill="${color}" font-weight="bold">${Math.floor(item.p.degree)}°</text>`;
      });
    });
    return out;
  }
  ...
  function buildAspects(data) {
    ...
    const pList = Object.keys(planets).map(k => {
      ...
      const abs = (signToDeg(p.sign) + parseFloat(p.degree) + (p.minute || 0) / 60) % 360;
      return { k, abs, vis: getVisualDeg(abs, ascAbs) };
    }).filter(x => x);
  ...
  function resolveNatalData(state) {
    const sc = window.totalShadowContext?.natal_context || {};
    const planets = sc.planets || state?.user?.natalPlanets || {};
    const houses = sc.houses || state?.user?.houses || {};
    let ascAbs = 0;
    const rawAsc = sc.ascendant_longitude ?? sc.ascendant_degree_absolute ?? state?.user?.ascDegAbs;
    if (rawAsc !== undefined && !isNaN(parseFloat(rawAsc))) ascAbs = parseFloat(rawAsc);
    else {
      const s = sc.ascendant || state?.user?.asc || 'Aries';
  ...
  window.renderChart650 = function (state) {
    const data = resolveNatalData(state);
    ...
    // Disparar evento para v2 de forma asíncrona para asegurar que el DOM se actualice
    setTimeout(() => {
